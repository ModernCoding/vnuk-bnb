...
public class BookingServiceDao {
	...
	@Autowired
	public BookingServiceDao(JdbcTemplate jdbcTemplate) {
		this.jdbcTemplate = jdbcTemplate;
	}

	// CREATE
	public void create(BookingService task) throws SQLException {
		...
		try {
			System.out.println(String.format("%s new record in DB!",
					...
					this.jdbcTemplate.update(sqlQuery,
							new Object[] { task.getBookingId(), task.getServiceId(), task.getPrice() })));
	...
	// READ (List of Tasks)
	public List<BookingService> read(String bookingId, String serviceId) throws SQLException {

		String sqlQuery = "select t01.id" + "     , t02.id as booking_id" + "     , t02.check_in "
				+ "     , t02.check_out " + "     , t02.quanlity_of_people " + "     , t03.id as service_id"
				+ "     , t03.label " + "     , t01.price " + "  from bookings_services t01, bookings t02, services t03"
				// + "join rooms_equipments t01 on t02.id = t01.room_id"
				+ " where t02.id = t01.booking_id" + " and t03.id = t01.service_id";
	...
	// READ (Single Task)
	public BookingService read(Long id) throws SQLException {

		String sqlQuery = "select t01.id" + "     , t02.id as booking_id" + "     , t02.check_in "
				+ "     , t02.check_out " + "     , t02.quanlity_of_people " + "     , t03.id as service_id"
				+ "     , t03.label " + "     , t01.price " + "  from bookings_services t01, bookings t02, services t03"
				+ "	 where t01.id = ?" + "  and t02.id = t01.booking_id" + "  and t03.id = t01.service_id"
				+ " order by t03.id asc, t02.id asc, t01.id asc" + ";";

		return this.jdbcTemplate.queryForObject(sqlQuery, new Object[] { id }, new BookingsServicesRowMapper());

	}

	// UPDATE
	public void update(BookingService task) throws SQLException {
		String sqlQuery = "update bookings_services set booking_id=?, service_id=?,price=? where id=?";

		try {
			this.jdbcTemplate.update(sqlQuery,

					new Object[] { task.getBookingId(), task.getServiceId(), task.getPrice(), task.getId() });
	...
	public void complete(Long id) throws SQLException {
		...
		BookingService task = this.read(id);
