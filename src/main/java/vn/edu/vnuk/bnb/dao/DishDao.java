...
public class DishDao {
	...
	public void create(Dish task) throws SQLException {
		...
		try {
			System.out.println(String.format("%s new record in DB!",
					...
					this.jdbcTemplate.update(sqlQuery, new Object[] { task.getPrice(), task.getDishTypeId(),
							task.getLabel(), task.getDescription() })));
	...
	public List<Dish> read(String dishTypeId) throws SQLException {
		...
		String sqlQuery = "select t01.id" + "     , t01.price" + "     , t02.id as dish_type_id" + "     , t01.label"
				+ "     , t01.description" + "     , t02.label" + "  from dishes t01, dish_types t02"
	...
	public Dish read(Long id) throws SQLException {
		...
		String sqlQuery = "select t01.id" + "     , t01.price" + "     , t02.id as dish_type_id" + "     , t01.label"
				+ "     , t01.description" + "     , t02.label " + "  from dishes t01, dish_types t02"
				+ " where t01.id = ?" + "   and t02.id = t01.dish_type_id" + " order by t02.id asc, t01.id asc" + ";";
		return this.jdbcTemplate.queryForObject(sqlQuery, new Object[] { id }, new DishesRowMapper());
	...
	public void update(Dish task) throws SQLException {
		...
		try {
			this.jdbcTemplate.update(sqlQuery,
					...
					new Object[] { task.getPrice(), task.getDishTypeId(), task.getLabel(), task.getDescription(),
							task.getId() });
