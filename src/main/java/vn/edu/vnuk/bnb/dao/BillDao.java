...
public class BillDao {
	...
	// CREATE
	public void create(Bill task) throws SQLException {
		...
		try {
			System.out.println(String.format("%s new record in DB!",
					...
					this.jdbcTemplate.update(sqlQuery, new Object[] { task.getBookingId(), task.getUserId(),
							task.getTotalPrice(), task.getCreated(), task.getUpdated() })));
	...
	// READ (List of Tasks)
	public List<Bill> read(String bookingId, String userId) throws SQLException {

		String sqlQuery = "select t01.id" + "     , t02.id as booking_id" + "     , t02.check_in "
				+ "     , t02.check_out " + "     , t02.quanlity_of_people " + "     , t01.total_price"
				+ "     , t03.id as user_id" + "     , t03.first_name " + "     , t03.middle_name "
				+ "     , t03.last_name " + "     , t03.address " + "     , t03.email " + "     , t03.phone "
				+ "     , t03.identification_number " + "     , t02.create_at " + "     , t02.update_at "
				+ "     , t01.created" + "     , t01.updated" + "  from bills t01, bookings t02, users t03"
				// + "join rooms_equipments t01 on t02.id = t01.room_id"
				+ " where t02.id = t01.booking_id" + " and t03.id = t01.user_id";
	...
	// READ (Single Task)
	public Bill read(Long id) throws SQLException {
		...
		String sqlQuery = "select t01.id" + "     , t02.id as booking_id" + "     , t02.check_in "
				...
				+ "     , t03.identification_number " + "     , t02.create_at " + "     , t02.update_at "
				+ "     , t01.created" + "     , t01.updated" + "  from bills t01, bookings t02, users t03"
				+ "	 where t01.id = ?" + "  and t02.id = t01.booking_id" + "  and t03.id = t01.user_id"
				+ " order by t03.id asc, t02.id asc, t01.id asc" + ";";

		return this.jdbcTemplate.queryForObject(sqlQuery, new Object[] { id }, new BillsRowMapper());

	}

	// UPDATE
	public void update(Bill task) throws SQLException {
		...
		try {
			this.jdbcTemplate.update(sqlQuery,
					...
					new Object[] { task.getBookingId(), task.getUserId(), task.getTotalPrice(), task.getCreated(),
							task.getUpdated(), task.getId() });
